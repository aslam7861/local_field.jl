...
function random_ali(L)
    if typeof(L) == FlintQadicField
       b = basis_qadic(L)
    else
       b = basis(L)
    end 
...
function norm_equation_iteration(L,a)
##{this solves the norm equation following induction as the proof of Falko, page-230}
   K = parent(a);
  ## @assert valuation(norm(uniformizer(L))) ==1
   piK = uniformizer(K);
   piL = uniformizer(L);
   f,mf = ResidueField(K);
   F,mF = ResidueField(L);
  #= _, b := normequation(F,mf(a));
   c := L!b;=#
   c = L(1) 
   C = [L(1)];   ##[c];
   n = valuation((a //  norm(C[1]))-1);
 #  repeat
            r = random_ali(L);
    while valuation(trace(r)) != 0 ||  valuation(r // L(trace(r))) !== 0
       ...
       z = ((a // norm(c))-1) // piK^n; 
       push!(C, 1+ piL^n* (z*r // L(trace(r))));
       c = prod(C);
     #//  c := c*(1+ piL^n* (z*r div Trace(r)) );
       n = valuation( (a// norm(c))-1);
    while precision(a) >= n+1 #   == n 
          z = ((a // norm(c))-1) // piK^n;
           push!(C, 1+ piL^n* (z*r // trace(r)));
       c = prod(C);
        n = valuation( (a// norm(c))-1);
   end
return c;
...
function base_field_ali(L) 
    if typeof(L) != FlintQadicField
      return base_ring(L.defining_polynomial)
    else
      return parent(norm(L(1)))
    end
end 

function absolute_ramification_index_ali(K) ##::LocalField{S, T}) where {S <: FieldElem, T <: LocalFieldParameter}
  d = ramification_index(K)
  L = base_field_ali(K)
  while absolute_degree(L) !=1 
       d *= ramification_index(L) 
       L = base_field_ali(L)
   end 
return d
end
...
function norm_equation_iteration_ramified(L,a) 

##{this solves the norm equation in totally ramified extension following induction, similar way of the proof of Falko, page-230}
   K = parent(a);
   if isinteger(absolute_ramification_index_ali(L)//prime(L))  ##  valuation(norm(uniformizer(L))) ==1
     error("This does not work for wild ramified part. Use exp and log")
   end   
   ...
   F,mF = ResidueField(L);
 ##  _, b := NormEquation(F,mf(a));
   m = ramification_index(L);
   F_inv = [a for a in F if a !=0]; 
   w =[x for  x in F_inv if  x^m == F(mf(a))];
   if  length(w) == 0 
      error("No soltion of norm equation");
   end
   c = pseudo_inv(mF)(w[1]);
   C = [c];
   n = valuation((a // norm(C[1]))-1);
    ...
    while valuation(trace(r)) != 0 &&  valuation(r // L(trace(r))) !== 0
      ...
      z = ((a//norm(c))-1);
      push!(C, 1+ (z*r// L(trace(r))));    ##confirm linearity of trace
       c = prod(C);
      n = valuation( (a//norm(c))-1);
    while precision(a) >= n+1   
